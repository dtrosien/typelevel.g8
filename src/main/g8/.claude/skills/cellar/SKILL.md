...
This project uses sbt 2 (project id `root`). cellar 0.1.0-M12 cannot read
sbt 2's virtual classpath (`--debug` shows "loading 1 jar(s)" and every
symbol is reported as not found), so the project-aware commands do not work
here. Use the `-external` commands with the coordinates from `build.sbt`.
Transitive dependencies are resolved, so the toolkit aggregate covers cats,
cats-effect, fs2, http4s, circe and decline:

```sh
cellar get-external org.typelevel:toolkit_3:$toolkit_version$ cats.effect.IO
cellar list-external org.typelevel:toolkit_3:$toolkit_version$ fs2.io.file
cellar get-external org.typelevel:toolkit-test_3:$toolkit_version$ weaver.MutableIOSuite
cellar get-external org.typelevel:weaver-scalacheck_3:$weaver_version$ weaver.scalacheck.Checkers
```

`weaver.SimpleIOSuite` and `weaver.IOSuite` are type aliases in weaver's
package object and cellar cannot resolve them. Look up the underlying traits
`weaver.SimpleMutableIOSuite` and `weaver.MutableIOSuite` instead.
